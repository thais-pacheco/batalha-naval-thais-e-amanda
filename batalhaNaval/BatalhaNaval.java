...
public class BatalhaNaval {
    ...
    public static void main(String[] args) {
        ...
        // Exibindo os tabuleiros de ambos os jogadores
        mostrarTabuleiro(tab1, nome1);
        mostrarTabuleiro(tab2, nome2);
        ...
        while (!valido) {
            try {
                ...
                if (entrada.length() < 2 || entrada.length() > 3) {
                    System.out.println("⚠️ Entrada inválida. Use formato como B7.");
                    continue;
                }

                char letra = entrada.charAt(0);
                if (letra < 'A' || letra > 'J') {
                    System.out.println("⚠️ Coluna inválida. Use letras de A a J.");
                    continue;
                }
                coluna = letra - 'A';
                ...
                if (linha < 0 || linha >= TAMANHO) {
                    System.out.println("⚠️ Linha inválida. Use números de 1 a 10.");
                    continue;
                }

                if (visivel[linha][coluna] == TIRO || visivel[linha][coluna] == ERRO) {
                    System.out.println("⚠️ Posição já atingida!");
                } else {
                    valido = true;
                }
            } catch (Exception e) {
